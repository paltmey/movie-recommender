...
def load_ratings(file_path):
    ...
    with open(file_path) as file:
        ...
        for row in tqdm(reader, total=line_count, desc=f'Loading {file_name}...'):
            user_id = int(row[0])
            movie_id = int(row[1])
            rating = float(row[2])
            timestamp = int(row[3])
            user_ratings = all_user_ratings.get(user_id, [])
            user_ratings.append({'movie_id': movie_id, 'rating': rating, 'timestamp': timestamp})
...
def filter_top_movies(user_ratings, movie_rating_counts, n):
    ...
    top_movies = set([item[0] for item in sorted_movie_rating_counts[:n]])
...
def sort_ratings(user_ratings):
    ...
    for user_id, ratings in tqdm(user_ratings.items(), desc='Sorting ratings'):
        ratings.sort(key=lambda entry: entry['timestamp'], reverse=True)
        sorted_user_ratings[user_id] = ratings

    return sorted_user_ratings


def load_movie_links(movie_links_path):
    print('Reading movie links...')

    movie_links = {}

    with open(movie_links_path) as file:
        reader = csv.reader(file)
        next(reader)  # skip header
        for row in reader:
            id = int(row[0])
            imdb_id = row[1]
            tmdb_id = row[2]
            movie_links[id] = {'imdb': imdb_id, 'tmdb': tmdb_id}
...
def load_movie_titles(movie_titles_path):
    ...
    with open(movie_titles_path) as file:
        reader = csv.reader(file)
        next(reader)  # skip header
        for row in reader:
            id = int(row[0])
            ...
            title = title_year[:-7]
            year = title_year[-5:-1]
            movie_titles[id] = {'title': title, 'year': year, 'genres': genres}

    return movie_titles


def build_vocab(movies, movie_titles_path, movie_links_path):
    movie_titles = load_movie_titles(movie_titles_path)
    movie_links = load_movie_links(movie_links_path)

    return {i: {'id': id, 'title': movie_titles[id]['title'], 'year': movie_titles[id]['year'], 'genres': movie_titles[id]['genres'], 'imdb': movie_links[id]['imdb'], 'tmdb': movie_links[id]['tmdb']} for i, id in
            enumerate(sorted(movies))}


def get_reverse_vocab(vocab):
    return {v['id']: k for k, v in vocab.items()}
...
def transform_data(data, vocab):
    sequences = []
    for sequence in data:
        sequences.append([vocab[entry['movie_id']] for entry in sequence])
...
def generate_dataset(data_dir, dataset_name, output_path, top_movie_n, chunksize, min_ratings,
                     split_percentage, shard_size, generate_check_dataset, check_dataset_cutoff):
    data_path = Path(data_dir)

    ratings, movie_ratings_count = load_ratings(data_path.joinpath('ratings.csv'))
    ...
    vocab = build_vocab(top_movies, data_path.joinpath('movies.csv'), data_path.joinpath('links.csv'))
    reverse_vocab = get_reverse_vocab(vocab)
    sequences, labels = generate_rating_sequences(sorted_ratings, reverse_vocab, chunksize, min_ratings)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Script to generate a tf record dataset for the MovieLens dataset.')
    parser.add_argument('--movielens_data_dir', type=str, default='data/raw/ml-25m')
    parser.add_argument('--dataset_name', type=str, default='movielens')
    parser.add_argument('--output_path', type=str, default='data/processed/movielens')
    parser.add_argument('--top_movie_n', type=int, default=5000)
    ...
    generate_dataset(args.movielens_data_dir, args.dataset_name, output_path, args.top_movie_n, args.chunksize,
